    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Geocoding"):
        ...
        for col in ranked_columns:
            if pd.notna(row.get(col)) and str(row[col]).strip():
                ...
                enriched = get_geocoded_info(query)
...
#        - Prefer columns that contain exact employee counts like "Employees Count (LinkedIn)"
#        - Do NOT map to ranges like "Headcount range"
#        - Do NOT map to number of followers, likes, or similar 
#     7. Always return mappings in format: Column Name -> Standard Field Name
#     8. If a column can be mapped to multiple standard fields, choose the most relevant one.
#     """
...
#     def recheck(mapping):
#         check_prompt = f"""
#         Recheck the following mapping output for correctness:
#         {mapping}, and the following requirements:
#         {STANDARD_BUSINESS_FIELDS}
        
#         Important rules:
#         1. Do not map number of employees to headcount range or similar, if we have any other mapping.
#         2. Just recheck the mapping and do not change the format.
#         3. If you find any errors, correct them.
#         """
...
#                         mappings[col] = field
#                 elif field not in mappings.values():
#                     mappings[col] = field

#     result_df = pd.DataFrame()
#     for field in STANDARD_BUSINESS_FIELDS:
#         matching_cols = [col for col, f in mappings.items() if f == field and col in df.columns]
#         # print(f"Matching columns for '{field}': {matching_cols}")

#         if field == 'Contact Name' and matching_cols:
#             for col in matching_cols:
#                 full_name = df[col].astype(str)
#                 if full_name.notnull().any():
#                     result_df['Contact First Name'] = full_name
#                     result_df['Contact Last Name'] = pd.Series([np.nan] * len(df), index=df.index)
#                     break
#             else:
#                 result_df['Contact First Name'] = pd.Series([np.nan] * len(df), index=df.index)
#                 result_df['Contact Last Name'] = pd.Series([np.nan] * len(df), index=df.index)


#             print(result_df['Contact First Name'].head(), result_df['Contact Last Name'].head())
#         elif matching_cols:
#             for col in matching_cols:
#                 if df[col].notnull().any():
#                     result_df[field] = df[col]
#                     break
#             else:
...
#     print(result_df["Contact First Name"].head(3), result_df["Contact Last Name"].head(3))
...
def map_to_standard_fields(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    4. For contact information, look for columns with contact details (name, email, phone, etc.)
    5. For Contact names:
        - If full name is provided, assign it to "Contact First Name"
        - If first and last names are separate, assign them to "Contact First Name" and "Contact Last Name"
    6. For phone numbers, look for columns containing "phone", "contact", "telephone", or similar terms
    7. For Number of Employees:
       - Prefer columns that contain exact employee counts like "Employees Count (LinkedIn)"
       - Do Do NOT map to ranges like "Headcount range"
       - Do NOT map to number of followers, likes, or similar 
    8. Always return mappings in format: Column Name -> Standard Field Name
    9. If a column can be mapped to multiple standard fields, choose the most relevant one.
    """
    ...
    def recheck(mapping):
        ...
        Donot change the format of the mapping output or do NOT add any extra details in the output.      
        Important rules:
        1. Donot map number of employees to headcount range or similar, if we have any other mapping.
        2. Just recheck the mapping and do not change the format.
        3. Do NOT map more than one standard column to a single column in the input.
    ...
    # mapping_text = recheck(mapping_text)
